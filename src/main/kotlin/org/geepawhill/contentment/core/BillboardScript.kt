...
class BillboardScript : ScriptBuilder<BillboardScript>() {
    ...
    init {
        ...
        headerFormat = Format(
            primaryJumbo, TypeFace.font(header, 3.0, 1.0),
            TypeFace.color(JfxUtility.color(40, 40, 180), 1.0)
        )
...
//
    ...
    private fun alice() {
        ...
        buildChord()
        fade(.2, 3000.0)
        letters("Thereby very likely\n     creating a\n   collosal failure\n          on an\nintergalactic scale!").format(
            formats[Size.Gigantic, Color.Emphatic]
        ).centered(viewport.all().center()).fadeIn(3000.0)
        endChord()
        pause()
        wipe()
    }
...
""".trimIndent()
    ...
    private fun tBetters() {
        ...
        for (i in 1..7) {
            ...
            art("agileandbeyond/blueBall.png", 50.0).centered(viewport.point(randomX, randomY)).called("#ix$i")
                .fadeIn(1000.0)
...
""".trimIndent()
    ...
    private fun change() {
        buildChord()
        header("'Better'")
        art("billboard/better.jpg", 950.0, 800.0).at(TopLeft(50.0, 50.0)).fadeIn()
        endChord()
        pause()
        wipe()
    }
    ...
    override fun downcast(): BillboardScript {
        return this
    }
