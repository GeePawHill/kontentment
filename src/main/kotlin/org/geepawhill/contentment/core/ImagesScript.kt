...
class ImagesScript : ScriptBuilder<ImagesScript>() {
    ...
    private fun simpleChange() {
        assume(primaryLine)
        val change = connector()
        change.actor.from(Point(100.0, 450.0)).to(Point(1500.0, 450.0), true)
        change.appear()
        assume(secondaryLine)
        stroke(1500, 450, 1500, 350).appear()
        pause()
    }
...
""".trimIndent()
    ...
    override fun downcast(): ImagesScript {
        return this
    }
